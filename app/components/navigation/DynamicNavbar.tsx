...
const DynamicNavbar = () => {
    ...
    const { user, isAdmin, isStudent, signOut, adminLogout, userProfile } = useAuth();
    const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

    const isAdminPath = pathname?.startsWith('/admindashboard');

    // Define navigation items based on role and current path
    const getNavItems = (): NavItem[] => {
        // If admin is on an admin-related path, show admin navigation
        ...
        // If either a student or an admin is on a student-facing path
        if (isStudent || isAdmin) {
            ...
            // If admin is browsing student pages, provide a quick link back to Admin Panel
            if (isAdmin) {
                items.push({ label: 'Admin Panel', href: '/admindashboard', icon: 'ShieldCheckIcon' });
            }
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="hidden lg:block">
                                        <p className="font-caption text-sm font-medium text-foreground">
                                            {userProfile?.name || user?.displayName || (isAdmin ? 'Administrator' : 'Student')}
                                        </p>
                                        <p className="font-caption text-xs text-muted-foreground">
                                            {(!isAdminPath && userProfile) ? `${userProfile.tokens} tokens` : (isAdmin ? 'Admin Account' : `${userProfile?.tokens || 0} tokens`)}
                                        </p>
                                    </div>
                                </div>
